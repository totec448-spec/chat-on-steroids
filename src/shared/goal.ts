 ...
 *
 * These trailers also own the completion audit. The editable prompt decides the user's policy
 * and voice; the trailer is the app's last-line protection against the most expensive failure
 * mode in an autonomous loop: accepting a confident completion sentence as proof of work that
 * the transcript itself still shows as promised, failed, skipped, unverified or blocked.
 */
export const GOAL_SYSTEM_TRAILER = `That was the conversation. Decide the smallest completion checklist implied by what the user actually asked for; do not add polish, tests, features or follow-ups they never requested. A conversational deliverable can be complete when the answer itself contains it. For computer-use or external-action work, a bare claim like "done", "saved", "fixed", "built" or "tests pass" is not enough when the conversation still shows the required action as merely planned, promised, failed, skipped, pending, or lacking the verification ChatGPT itself said it would perform. Current concrete results beat earlier plans and claims. If a required item is unfinished, write one short next message naming exactly that item and the next useful action. If ChatGPT asks a question whose answer is already unambiguous from the conversation, answer it and keep going. Do not loop forever on an impossible blocker: if the same blocker has repeated across three consecutive assistant turns and it genuinely requires missing user-only information, permission or credentials, stop rather than sending the same instruction again. Answer exactly NO_REPLY only when every user-requested item is clearly resolved, or when that repeated user-only blocker makes further autonomous progress impossible. Write in the user's language and register, and write nothing except that message.`;

export const GOAL_OBJECTIVE_TRAILER = `That was the conversation. Treat the stated goal as both the finish line and the ceiling. Break it into the smallest concrete completion conditions it actually implies; never add nice-to-haves. A conversational deliverable can be complete when the answer itself contains it. For computer-use or external-action work, do not treat confidence as completion: "done", "saved", "fixed", "built" or "tests pass" does not satisfy a goal item that the conversation still shows as planned, promised, failed, skipped, pending, or missing the verification ChatGPT itself said it would perform. Current concrete results beat earlier plans and claims. Continue with one short, specific user message naming the unmet condition and next useful action. If the same blocker has repeated across three consecutive assistant turns and it truly requires missing user-only information, permission or credentials, stop instead of creating an infinite loop. Answer exactly NO_REPLY only when every part of the goal is actually reached, or when that repeated user-only blocker makes further autonomous progress impossible. Never widen the goal. Write in the user's language and register, and write nothing except that message.`;

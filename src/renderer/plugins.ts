...
function showConnection(): void {
  ...
  actions.append(button(() => t("Open ChatGPT plugins"), async () => { await run(window.api.openLink('https://chatgpt.com/#settings/Plugins')); }), button(() => t("Save & connect"), async () => {
    ...
    if (tunnel) {
      const { capabilities, readOnly, tunnel: previousTunnel, ui, sessions, compaction, multiAgent, remoteSteering, goal, mcp } = appState.config;
      const base: SettingsPatch = { capabilities, readOnly, tunnel: previousTunnel, ui, sessions, compaction, multiAgent, remoteSteering, goal, mcp };

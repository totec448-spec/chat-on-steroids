...
function hasTranslation(source: string): boolean {
  return Object.values(catalogs).some(catalog => Object.hasOwn(catalog, source));
}

let language: Language = 'en';
try { language = resolveLanguage(window.localStorage.getItem(STORAGE_KEY)); } catch { /* Storage may be unavailable in a restricted renderer. */ }
...
export function initLanguage(): void {
  ...
  for (const node of texts) {
    ...
    if (hasTranslation(key)) ui(node, 'textContent', () => source.replace(/\S[\s\S]*\S|\S/, t(key)));
  }
  for (const node of document.querySelectorAll<HTMLElement>('[title], [placeholder], [aria-label]')) {
    for (const property of ['title', 'placeholder', 'aria-label'] as const) {
      ...
      if (source && hasTranslation(source)) ui(node, property, () => t(source));
    }
  }
  document.documentElement.lang = language;
  const select = document.getElementById('uiLanguage') as HTMLSelectElement;
  syncLanguageControls();
  select.addEventListener('change', () => setLanguage(resolveLanguage(select.value)));
  for (const button of document.querySelectorAll<HTMLButtonElement>('[data-language]')) {
    button.addEventListener('click', () => setLanguage(resolveLanguage(button.dataset.language)));
  }
}

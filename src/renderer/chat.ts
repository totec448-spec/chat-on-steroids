...
function paintDeliveryControls(): void {
  ...
  if (sendOption) ui(sendOption, 'textContent', () => canInject ? t("Inject now") : t("Send"));
...
function compactionState(block: CompactionBlock): { text: string; tone: CompactionTone } {
  ...
  if (block.handoff) {
    return block.moved
      ? { text: t("Summary saved{0}, but no new chat was opened — the chat carried on here", [chars]), tone: 'bad' }
      : { text: t("Summary saved{0} — opening the new chat…", [chars]), tone: 'wait' };
  }
  if (block.brief?.final) {
    return block.moved
      ? { text: t("Summary written, but the app never saved it — the chat carried on here"), tone: 'bad' }
      : { text: t("Summary written — saving the handoff…"), tone: 'wait' };
  }
  if (block.brief) return { text: t("ChatGPT is writing the summary…"), tone: 'wait' };
  return block.moved
    ? { text: t("No summary was written — the chat carried on here"), tone: 'bad' }
    : { text: t("Summary requested — waiting for ChatGPT…"), tone: 'wait' };
}
...
function compactionRow(block: CompactionBlock): HTMLElement {
  ...
  head.append(el('span', 'state', () => compactionState(block).text));
  box.append(head);
...
async function sendComposer(delivery?: 'finish', plan?: string[], planObjective?: string): Promise<boolean | void> {
  ...
  try {
    ...
    if (!result) {
      if (selectedId === sessionId && selectionGeneration === generation && !input.value) { input.value = authoredDraft; resizeComposer(); }
    ...
    if (selectedId === sessionId && selectionGeneration === generation) resizeComposer();
    void refreshInputQueue();
    return true;
...
function showView(name: string): void {
  ...
  if (name !== 'settings') resizeComposer();

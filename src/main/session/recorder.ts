...
async function promoteConversationTitle(sessionId: string, title?: string): Promise<void> {
  ...
  const fallback = firstUser?.kind === 'user_message' ? (firstUser.authoredText ?? userPromptText(firstUser.message.text) ?? firstUser.message.text).trim().slice(0, 80) : '';
  // An earlier Markdown observation could name the session before its app receipt
  // supplied authoredText. Only that exact durable fallback is eligible for repair.
  const observedFallback = firstUser?.kind === 'user_message' ? firstUser.message.text.trim().slice(0, 80) : '';
  if (!fallback || (summary.title !== fallback && summary.title !== observedFallback)) return;
  await renameSession(sessionId, next);
  notifyChanged();
}
...
function observedUserTitle(first?: string): string | undefined {
  ...
  const authored = userPromptText(first);
  // Native Markdown can consume whitespace in the frame. It is not safe to parse
  // that as wire text, and a transport header is never a useful fallback title.
  if (authored === null && /^\[\[COS_CONTEXT:\d{1,6}\]\]/.test(first)) return undefined;
  return (authored ?? first).slice(0, 80);
}
...
async function recordChatObservationsNow(
  ...
  let recoveredFinal: { turnId: string; time: number; seq: number; origin: number } | undefined;
      ...
      case 'assistant_message': {
        ...
        if (state === 'final' && written.event.kind === 'assistant_message' && canonicalTurn && recoverableTurns.has(canonicalTurn) &&
          ...
          recoveredFinal = { turnId: canonicalTurn, time: item.time,
            ...
            origin: written.event.origin ?? written.event.seq };
        }
        if (
  ...
  if (recoveredFinal && latestWork && latestWork.seq < recoveredFinal.seq &&

  ...
  try {
    if (handle) {
      ...
      for (;;) {
        ...
        // `chunk` is reused by the next read. An incomplete JSONL row must own its bytes;
        // otherwise the next read overwrites the carried prefix in place and a large event can
        // disappear from the reconstructed session summary after restart/compaction recovery.
...
/**
 * Reads the oldest matching presentation rows without materialising the complete journal.
 *
 * Goal/Loop needs the true original user request even when the recent reverse reader exhausts
 * its byte budget long before it reaches the start of a very large session. Canonical website
 * messages can have a late revision `seq` but an early immutable `origin`, so seed the result
 * from the canonical map first, then scan the append-only journal forward only until no later
 * raw row can beat the oldest rows already found.
 */
export async function readOldestEvents(
  sessionId: string,
  limit: number,
  ...
  const active = open.get(sessionId);
  const needsMessages = !options.kinds || options.kinds.includes('user_message') ||
    options.kinds.includes('assistant_message') || options.kinds.includes('tool_call');
  const messages = needsMessages
    ? active?.messages ?? (await readCanonicalMessages(sessionId))
  ...
  const add = (event: SessionEvent): void => {
    ...
    selected.push(event);
    selected.sort((left, right) => positionOf(left) - positionOf(right) || left.seq - right.seq);
    if (selected.length > cap) selected.length = cap;
  };

  for (const message of messages.values()) add(message);
  let cutoff = selected.length >= cap ? positionOf(selected[selected.length - 1]!) : Number.POSITIVE_INFINITY;
  ...
  try {
    ...
    let done = false;
    const accept = (line: Buffer): void => {
      if (done || line.length === 0 || line.length > MAX_LINE_BYTES) return;
      let parsed: SessionEvent;
      try {
      ...
      if (typeof parsed?.seq !== 'number' || typeof parsed?.kind !== 'string') return;
      // Journal sequence is monotonic. Once it moves beyond the worst presentation position
      // already retained, only a canonical revision could point further back — and those were
      // seeded from `messages` above before this scan began.
      if (selected.length >= cap && parsed.seq > cutoff) {
        done = true;
        return;
      }
      ...
      add(parsed);
      cutoff = selected.length >= cap ? positionOf(selected[selected.length - 1]!) : Number.POSITIVE_INFINITY;
    };

    while (!done && filePosition < readBudget) {
      const wanted = Math.min(chunk.length, readBudget - filePosition);
      const { bytesRead } = await handle.read(chunk, 0, wanted, filePosition);
      if (bytesRead === 0) break;
      ...
      // `chunk` is reused by the next read. Keep an owned copy of an incomplete line;
      // otherwise a >64 KiB message can have its carry bytes overwritten in-place by
      // the following read and the reconstructed oldest row silently acquires later text.
      carry = done ? Buffer.alloc(0) : Buffer.from(joined.subarray(start));
      if (carry.length > MAX_LINE_BYTES) carry = Buffer.alloc(0);
    }

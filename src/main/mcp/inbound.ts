...
const store = new AsyncLocalStorage<{ requestId: string | null; timing?: InboundTiming }>();
...
export function withInboundRequestId<T>(requestId: string | null, body: () => T, timing?: InboundTiming): T {
  return store.run({ requestId, timing }, body);
}

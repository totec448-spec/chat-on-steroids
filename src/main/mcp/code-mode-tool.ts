...
export const codeModeDeclaration = (options: CodeModeOptions = {}) => toolDeclaration('exec', () => ({
  ...
  description: options.windowsDesktop
    ? 'Run Windows Computer Use JavaScript with sky: list_apps, list_windows, get_window, launch_app, get_window_state, click, press_key, type_text, scroll, set_value, drag, perform_secondary_action, activate_window. Same arguments as the direct tools; sky returns native values and throws tool errors. get_window_state automatically displays its screenshots. Use nodeRepl.write(value) or text(value) for text. sky.target is windows. The app supplies sky; no import is needed. Fresh bounded runtime per call: variables do not persist; retain returned Window objects using get_window across calls. tools.<name> remains available with MCP result objects; ALL_TOOLS lists available methods. No Node/filesystem/network. 64k source, 32 MiB JS memory, 2s CPU, 60s total, 32 calls, 8 concurrent, 4 images and 12 MiB output. Exact companion identity and live tool permissions apply. Observe and inspect before choosing an action; then act and refresh. Do not repeat an input after a refresh failure.'

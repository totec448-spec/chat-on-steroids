...
function recognizable(tools: unknown, surface: PluginSurface = 'core'): tools is PluginToolSchema[] {
  if (!Array.isArray(tools) || (!tools.length && surface !== 'plugins') || tools.length > (surface === 'plugins' ? PLUGIN_MAX_TOOLS : 16) || JSON.stringify(tools).length > 300000) return false;
...
function enrollable(tools: unknown, publication: PluginPublication): boolean {
  ...
  // that stale Plugins subset only when every installed declaration is an exact
  ...
  if (publication.surface === 'plugins' && tools.length === LEGACY_PLUGIN_MAX_TOOLS && publication.tools.length > tools.length) {

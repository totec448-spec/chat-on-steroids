...
// Split since this branch was written: the shared file is a registrar now, and the macOS
// contract this asserts lives in its own.
...
describe('macOS Computer Use v16 input routing', () => {
  ...
  it('fails closed for physical mutations without an exact WindowLease', () => {
    // The option is `window` — the name this repository already gave it for Windows input — and
    // the helper protocol's field stays `targetWindow`, which is also what the tool exposes.
    expect(computer).toContain('window?: number; ownerWindow?: number');
  ...
  it('exposes closed-loop targetWindow and automatic result capture', () => {
    ...
    expect(computer).toContain('capture.window =\n        result.targetWindow ??');

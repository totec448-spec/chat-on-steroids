...
Source alignment: **2026-09-10**, including current working-tree changes. App/extension **2.0.8**,
...
- Waiting chats and reusable sleepers stay available. Closing needs terminal, blocked,
  cancelled, superseded or proven-duplicate authority plus fresh document/draft/generation
  checks. User-pinned tabs veto every automatic close, including pinning during the proof.
  Blocked/retired status never overrides active generation. Idle time and worker-slot pressure
  alone do not grant closure.
...
all of them to the model. App-authored executor sends therefore use `session/prompt.ts` to freeze
the complete current Core instructions plus the explicitly linked project's root `AGENTS.md`
inside the existing `COS_CONTEXT` frame before handout. Worker/revival/resume and handoff-request
producers use that same preparation authority. Internal decision/planner prompts keep their
separate role-specific contract. Direct user sends in Chrome are not intercepted.
...
byte envelope. Authored work and complete Core instructions are mandatory; only AGENTS content
spends remaining room. Read it as a bounded UTF-8 prefix, validate the same project/root/read
...
native source text; rendered Markdown whitespace alone cannot prove a send.
...
adapter automatically emits its returned screenshots. `sky` calls the same registered tools,
...
prime; Resume uses the same session. Unfiled chats receive no project file. Missing AGENTS is
fine; an unsafe/unreadable/binary file fails preparation visibly. Full prompt budgeting and
...
The frozen `deliveryText` includes the full executor envelope at claim time; displayed authored
...
| --- | --- |
| Immediate / `auto` | When exact active work can receive tools, inject into a later eligible outer MCP result. For a proven idle chat/New Chat, elect the normal browser send. Ambiguous activity is not proof of an idle composer. |
...
or fresh-chat ownership. It confirms model/effort, inserts the full text, uploads native files,
...
election additionally obeys the continuation WAL (§15). Closing any helper/source/duplicate
requires operation-specific retirement authority and fresh idle/draft/document checks.
...
Unattributed recovery uses staged 15/30/60-second incident deadlines according to live suspects;
...
The MV3 30-second maintenance alarm only collects already-authorized work.
...
then durably reserves the worker before handing out browser work. Shared context carries common
project/instructions; each worker gets its bounded assignment. Invitations, active workers,
...
and browser close are separate: preserve waiting/sleeping tabs, and close only proven retired
or duplicate documents after idle/draft checks. Compact & Resume rebinds a prime within its
...
original native owner. Public coordinates are window-relative logical pixels, converted through
the selected screenshot's origin/scale and target DPI. Named punctuation follows the target
...
browser-chord policy prevents tab/window/address-bar management through forbidden input chords;

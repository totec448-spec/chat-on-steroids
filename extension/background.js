...
async function pruneManagedTabs(tabs, policy, protectedChats, closable) {
  ...
  for (const tab of candidates) {
    ...
    try {
      const current = await chrome.tabs.get(tab.id);
      if (current.pinned || conversationFromUrl(current.url) !== conversationId || current.pendingUrl) continue;
      ...
      if (latest.pinned || latest.pendingUrl || conversationFromUrl(latest.url) !== conversationId || !ownsDocument(source) || journalCountForConversation(conversationId) > 0) continue;

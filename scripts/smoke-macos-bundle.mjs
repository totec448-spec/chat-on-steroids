...
for (const key of ['NSCameraUsageDescription', 'NSMicrophoneUsageDescription', 'NSAudioCaptureUsageDescription']) {
  const result = run('plutil', ['-extract', key, 'raw', plist], { allowFailure: true });
  if (result.status === 0) throw new Error(`Info.plist unexpectedly contains unused media privacy key ${key}`);
}

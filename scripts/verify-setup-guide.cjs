...
app.whenReady().then(async () => {
  ...
  try {
    ...
    for (const [width, height, zoom, language, theme] of [
      ...
      [1100, 900, 1, 'de', 'dark'], [1100, 900, 1.5, 'de', 'light'], [640, 720, 1, 'de', 'dark']
    ]) {
    ...
    await key('Tab'); await key('Space');
    assert.equal(await win.webContents.executeJavaScript('document.documentElement.lang'), 'de');
    await win.loadURL(server.resolvedUrls.local[0] + 'setup-preview.html');
    assert.deepEqual(await win.webContents.executeJavaScript(`({language:document.documentElement.lang,
      selected:document.querySelector('[data-language="de"]').getAttribute('aria-pressed'),
      preference:document.getElementById('uiLanguage').value})`), {language:'de', selected:'true', preference:'de'});
